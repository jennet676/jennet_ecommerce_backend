...
export async function importSingle(req, res) {
  try {
...
export async function importArray(req, res) {
  try {
...
export async function importMultiple(req, res) {
  try {
    return res.status(201).send({
      images: req.files,
      message: "Images uploaded successfully",
...
export async function deleteFile(req, res) {
  try {
    ...
    if (!fs.existsSync(`images/${filename}`)) {
      return res
        .status(400)
        .send({ message: `the filename ${filename} does not exists!` });
    }
